...
const version = `/* a11y_datetime v${pkg.version}, based on flatpickr, @license MIT */`;
...
async function buildScripts() {
  try {
    ...
    const transpiled = await readFileAsync("./dist/a11y_datetime.js");
    const minified = await uglify(transpiled);

    await Promise.all([
      fs.writeFile("./dist/a11y_datetime.min.js", minified),
      // Backward compatible aliases
      fs.copy("./dist/a11y_datetime.js", "./dist/flatpickr.js"),
      fs.writeFile("./dist/flatpickr.min.js", minified),
    ]);
  } catch (e) {
    logErr(e);
  }
    ...
    await Promise.all(
      themePaths
        .filter((themePath) => {
          ...
          return SUPPORTED_THEMES.includes(themeName);
        })
        .map(async (themePath) => {
...
async function buildDemoPage() {
  const demoTemplatePath = "./index.template.html";
  const distDemoPath = "./dist/demo/index.html";

  const template = await readFileAsync(demoTemplatePath);
  ...
  await fs.writeFile(distDemoPath, distDemoHtml);
}
