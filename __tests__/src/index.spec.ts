...
describe("flatpickr", () => {
  ...
  describe("UI", () => {
    it("keyboard can open and select a date from input", () => {
      createInstance();

      fp._input.focus();
      simulate("keydown", fp._input, { keyCode: 13 }); // Enter

      const active = document.activeElement as HTMLElement;
      expect(active.classList.contains("flatpickr-day")).toBe(true);

      simulate("keydown", active, { keyCode: 32 }); // Space
      expect(fp.selectedDates.length).toBe(1);
    });

    it("month and year controls are keyboard focusable", () => {
      createInstance({
        monthSelectorType: "dropdown",
      });

      expect(fp.monthsDropdownContainer.tabIndex).toBe(0);
      expect(fp.currentYearElement.tabIndex).toBe(0);
    });

    it("tab from input reaches month and year controls", () => {
      createInstance();

      fp._input.focus();
      simulate("keydown", fp._input, { keyCode: 13 }); // Enter
      simulate("keydown", fp._input, { keyCode: 9 }); // Tab

      expect(document.activeElement).toBe(fp.prevMonthNav);

      simulate("keydown", fp.prevMonthNav, { keyCode: 9 }); // Tab
      expect(document.activeElement).toBe(fp.monthsDropdownContainer);

      simulate("keydown", fp.monthsDropdownContainer, { keyCode: 9 }); // Tab
      expect(document.activeElement).toBe(fp.currentYearElement);
    });

    it("tab to day grid focuses today first", () => {
      createInstance();

      fp._input.focus();
      simulate("keydown", fp._input, { keyCode: 13 }); // Enter
      simulate("keydown", fp._input, { keyCode: 9 }); // Tab -> prev
      simulate("keydown", fp.prevMonthNav, { keyCode: 9 }); // Tab -> month
      simulate("keydown", fp.monthsDropdownContainer, { keyCode: 9 }); // Tab -> year
      simulate("keydown", fp.currentYearElement, { keyCode: 9 }); // Tab -> next
      simulate("keydown", fp.nextMonthNav, { keyCode: 9 }); // Tab -> day

      expect(document.activeElement).toBe(fp.todayDateElem);
    });

    it("shift+tab from day moves focus back to next-month arrow", () => {
      createInstance();
      fp.open();
    ...
    it("pressing M focuses month control as keyboard fallback", () => {
      ...
      simulate("keydown", firstDay, { keyCode: 77 }); // M
    ...
    it("has a keyboard-focusable close button that closes calendar", () => {
      createInstance();
      fp.open();

      expect(fp.closeButton).toBeDefined();
      expect(fp.closeButton && fp.closeButton.tabIndex).toBe(0);

      clickOn(fp.closeButton as Node);
      expect(fp.isOpen).toBe(false);
    });
    ...
    it("can hide close button via showCloseButton option", () => {
      ...
      expect(fp.closeButton).toBeUndefined();
      expect(
        fp.calendarContainer.querySelector(".flatpickr-close-button")
      ).toBeNull();
    });

    it("can hide title bar when showTitleBar and showCloseButton are false", () => {
      createInstance({
        showTitleBar: false,
      ...
      expect(fp.calendarContainer.querySelector(".flatpickr-titlebar")).toBeNull();
    });

    it("renders a custom calendar title when provided", () => {
      createInstance({ calendarTitle: "Mein Terminplaner" });
      fp.open();

      const title = fp.calendarContainer.querySelector(".flatpickr-title");
      expect(title).toBeTruthy();
      expect(title && title.textContent).toBe("Mein Terminplaner");
    });

    it("sets an accessible dialog name from calendarTitle", () => {
      createInstance({ calendarTitle: "Accessible Planner" });
      fp.open();

      expect(fp.calendarContainer.getAttribute("role")).toBe("dialog");
      expect(fp.calendarContainer.getAttribute("aria-label")).toBe(
        "Accessible Planner"
      );
    });
    ...
    it("initialDayFocus selected prioritizes selected day", () => {
      const selectedDate = new Date(2020, 5, 15);
      createInstance({
        defaultDate: selectedDate,
        initialDayFocus: "selected",
      });
      ...
      expect(document.activeElement).toBe(fp.selectedDateElem);
    });
    ...
    it("mode: multiple keeps keyboard focus in calendar after selection", () => {
      createInstance({
        mode: "multiple",
      });
      ...
      simulate("keydown", firstDay, { keyCode: 13 }); // Enter
    ...
    it("month control respects minDate when navigating backwards", () => {
      const fp = createInstance({
